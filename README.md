...
- Worker-assisted Gear, Technique, Charm, Relic, Full Build, Chaos Realm, and Acquisition searches, with progress and cancellation where applicable.
...
- Data Health page with source provenance, exact/derived/override/missing status, and refresh diffs.
- Share links include the active build plus equipped current-season gear and relic loadout.
- Screenshot OCR for Current Build and PvP opponents with confidence review before applying fields.
- Team 4v4 AI trace showing the client targeting priority chain and when a fallback was required.
- Acquisition Planner ranks missing upgrades by modeled build value and uses exact drop/acquisition probabilities only when loaded.
...
Use **Reference → Data Health** to audit the current data state and the most recent source refresh diff. Static set rules and the Datamine Catalog are editable under **Reference → Set Logic**.
